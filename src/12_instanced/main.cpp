...
int main()
{
    ...
    glfwWindowHint(GLFW_SAMPLES, 4);                                // 开启 4x MSAA 支持的帧缓冲设置
    ...
    // glEnable(GL_MULTISAMPLE);       // 开启 MSAA，默认开启
    ...
    unsigned char *imageData = stbi_load("./src/12_instanced/container.png", &width, &height, &nrChannels, 0);
    if (imageData)
    {
    ...
    // 加载贴图
    stbi_set_flip_vertically_on_load(true); // tell stb_image.h to flip loaded texture's on the y-axis.
    imageData = stbi_load("./src/12_instanced/container_specular.png", &width, &height, &nrChannels, 0);
    if (imageData)
    {
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, imageData);
        glGenerateMipmap(GL_TEXTURE_2D);            // 手动开启多级渐远纹理，或者直接使用上面的 glTexImage2D(GL_TEXTURE_2D, n, ...)
    ...
    {
        "./src/12_instanced/skybox/right.jpg",
        "./src/12_instanced/skybox/left.jpg",
        "./src/12_instanced/skybox/top.jpg",
        "./src/12_instanced/skybox/bottom.jpg",
        "./src/12_instanced/skybox/front.jpg",
        "./src/12_instanced/skybox/back.jpg"
    };
    ...
    Shader outShader("./src/12_instanced/shader/vertex.glsl", "./src/12_instanced/shader/fragment.glsl");
    ...
    Shader skyboxShader("./src/12_instanced/shader/skybox_vertex.glsl", "./src/12_instanced/shader/skybox_fragment.glsl");
    skyboxShader.use();
    glActiveTexture(GL_TEXTURE0); 
    glBindTexture(GL_TEXTURE_CUBE_MAP, cubemapTexture);
    ...
    while (!glfwWindowShouldClose(window))
	{
        ...
        {
            glm::mat4 model = glm::rotate(cubeModelMatrixes[i], float(i + glfwGetTime()), glm::vec3(1.0, 0.5, 0.0));
            glBufferSubData(GL_ARRAY_BUFFER, i * mat4Size, mat4Size, glm::value_ptr(model));
        }
        glBindBuffer(GL_ARRAY_BUFFER, 0);
