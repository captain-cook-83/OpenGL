...
int main()
{
    ...
    Shader outShader("./src/13_framebuffers_mass/shader/vertex.glsl", "./src/13_framebuffers_mass/shader/fragment.glsl");
    ...
    unsigned char *imageData = stbi_load("./src/13_framebuffers_mass/container.png", &width, &height, &nrChannels, 0);
    if (imageData)
    {
    ...
    // 加载贴图
    stbi_set_flip_vertically_on_load(true); // tell stb_image.h to flip loaded texture's on the y-axis.
    imageData = stbi_load("./src/13_framebuffers_mass/container_specular.png", &width, &height, &nrChannels, 0);
    if (imageData)
    {
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, imageData);
        glGenerateMipmap(GL_TEXTURE_2D);            // 手动开启多级渐远纹理，或者直接使用上面的 glTexImage2D(GL_TEXTURE_2D, n, ...)
    ...
    // 降采样帧缓冲
    unsigned int dsFramebuffer;
    glGenFramebuffers(1, &dsFramebuffer);
    glBindFramebuffer(GL_FRAMEBUFFER, dsFramebuffer);

    unsigned int dsColorBuffer;
    glGenTextures(1, &dsColorBuffer);
    glBindTexture(GL_TEXTURE_2D, dsColorBuffer);
    ...
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, dsColorBuffer, 0);
    ...
    // 加载后处理 Shader
    Shader ppShader("./src/13_framebuffers_mass/shader/pp_vertex.glsl", "./src/13_framebuffers_mass/shader/pp_fragment.glsl");
    ...
    while (!glfwWindowShouldClose(window))
	{
        ...
        if (usePostProcess) {
            ...
            glBindFramebuffer(GL_DRAW_FRAMEBUFFER, dsFramebuffer);
            glBlitFramebuffer(0, 0, currentScrWidth, currentScrHeight, 0, 0, currentScrWidth, currentScrHeight, GL_COLOR_BUFFER_BIT, GL_NEAREST);
        ...
        ImGui::Checkbox("Use PostProcess", &usePostProcess);
        ImGui::End();
