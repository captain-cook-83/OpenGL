...
int main()
{
    ...
    Shader outShader("./src/05_transform/shader/vertex.glsl", "./src/05_transform/shader/fragment.glsl");
    ...
    unsigned char *imageData = stbi_load("./src/05_transform/container.jpg", &width, &height, &nrChannels, 0);
    if (imageData)
    {
    ...
    // 加载贴图
    imageData = stbi_load("./src/05_transform/awesomeface.png", &width, &height, &nrChannels, 0);
    if (imageData)
    {
    ...
    unsigned int transformLoc = glGetUniformLocation(outShader.ID, "transform");
