...
int main()
{
    ...
    Shader outShader("./src/08_light_base/shader/vertex.glsl", "./src/08_light_base/shader/fragment.glsl");
    ...
    unsigned char *imageData = stbi_load("./src/08_light_base/container.png", &width, &height, &nrChannels, 0);
    if (imageData)
    {
    ...
    // 加载贴图
    stbi_set_flip_vertically_on_load(true); // tell stb_image.h to flip loaded texture's on the y-axis.
    imageData = stbi_load("./src/08_light_base/container_specular.png", &width, &height, &nrChannels, 0);
    if (imageData)
    {
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, imageData);
        glGenerateMipmap(GL_TEXTURE_2D);            // 手动开启多级渐远纹理，或者直接使用上面的 glTexImage2D(GL_TEXTURE_2D, n, ...)
